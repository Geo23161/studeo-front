<template >
    <ion-page>
        <ion-content :scroll-y="false">
            ...
            <div class="serif" style="max-height: 100%; overflow-y: scroll; padding-bottom: 18rem;">
                <div class="cta_cont">
                    <div class="cta" v-if="!notifs.conseils.length">
                        <div class="sp_r">
                            <img :src="'../../imgs/conseil.svg'" style="height: 4.5rem;" />
                        </div>
                        <div>
                            ...
                            <div style="font-size: .8rem; line-height: .9rem; padding-top: .2rem;">
                                Bourses, opportunités et conseils, nos experts sont là pour vous.
                            </div>
                            <div class="ancre" @click="router.push('/cask')"
                                style="font-size: .9rem; font-weight: bold; color: #EFD109; padding-top: .4rem;">
                                Demande maintenant
                            </div>
                        </div>
                    </div>
                    <div class="cta" style="border-color: #EFD109 !important;" v-else>
                        <div class="sp_r">
                            <img :src="'../../imgs/notif_conseil.svg'" style="height: 4.8rem;" />
                        </div>
                        <div>
                            <div style="font-weight: bold; font-size: .9rem;">
                                Une réponse pour votre question
                            </div>
                            <div style="font-size: .8rem; line-height: .9rem; padding-top: .2rem;">
                                La dernière question que vous avez posée a déjà une réponse.
                            </div>
                            <div @click="router.push('/cquest/' + notifs.conseils[0])" class="ancre"
                                style="font-size: .9rem; font-weight: bold; color: #EFD109; padding-top: .4rem;">
                                Voir maintenant
                            </div>
                        </div>
                ...
                <div v-for="aide in conseils" :key="aide.id" class="res_cont">
                    <div @click="router.push('/cquest/' + aide.id)" class="question_h sp_r">
            ...
            <cta-comp :is-open="qOpen" :is-cons="true" :color="'#EFD109'"></cta-comp>
            <filter-comp :is-open="fOpen" :typ-o="'conseil'" :is-cons="true" @close="fOpen = false" :filt-obj="filtObj"
...
import { IonPage, IonContent, IonIcon, IonRippleEffect, onIonViewWillLeave, onIonViewDidEnter } from "@ionic/vue"
...
const excep = computed(() => {
    ...
    for(const elt of conseils.value) {
        pks.push(elt.id)
    }
...
const get_conseils = async () => {
    ...
    try {
        const resp = await axios.post('api/get_conseils/', {
        ...
        if (resp.data['done']) {
            conseils.value = conseils.value.concat(resp.data['result'])
