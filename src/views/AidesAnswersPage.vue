<template >
    <ion-page>
        <ion-content :scroll-y="false">
            ...
            <div class="serif" id="global_cont" style="max-height: 100%; overflow-y: scroll; padding-bottom: 18rem;">
                <div class="cta_cont">
                    <div class="cta" v-if="!notifs.aides.length">
                        <div class="sp_r">
                            <img :src="'../../imgs/aide.svg'" style="height: 4.5rem;" />
                        </div>
                        <div>
                            <div style="font-weight: bold; font-size: .9rem;">
                                Besoin d'aide pour tes exercices ou cours?
                            </div>
                            <div style="font-size: .8rem; line-height: .9rem; padding-top: .2rem;">
                                Pose ta question ou envoie-nous le problème ou l'exercice
                            </div>
                            <div @click="router.push('/ask?typ=aide')" class="ancre"
                                style="font-size: .9rem; font-weight: bold; color: #2DCF42; padding-top: .4rem;">
                                Demande maintenant
                            </div>
                        </div>
                    </div>
                    <div class="cta" style="border-color: #2DCF42 !important;" v-else>
                        <div class="sp_r">
                            <img :src="'../../imgs/notif_aide.svg'" style="height: 4.8rem;" />
                        </div>
                        <div>
                            <div style="font-weight: bold; font-size: .9rem;">
                            ...
                            <div class="ancre" @click="router.push('/quest/aide:' + notifs.aides[0])"
                                style="font-size: .9rem; font-weight: bold; color: #2DCF42; padding-top: .4rem;">
                                Voir maintenant
                            </div>
                        </div>
                ...
                <div v-for="aide in aides" :key="aide.id" class="res_cont">
                    <div @click="router.push('/quest/aide:' + aide.id)" class="question_h sp_r">
                        ...
                        <div v-if="aide.image.length || aide.audio.length || aide.video.length" class="sup_file">
                            ...
                            <div v-else-if="aide.audio.length" id='audio_f' @click.stop style="margin-top: -.7rem;">
            ...
            <cta-comp :is-open="qOpen"  :color="'#2DCF42'"></cta-comp>
            <filter-comp :is-open="fOpen" :typ-o="'aide'" @close="fOpen = false" :filt-obj="filtObj"
...
import { IonPage, IonContent, IonIcon, IonRippleEffect, IonSkeletonText, onIonViewWillLeave, onIonViewDidEnter } from "@ionic/vue"
...
const get_aides = async () => {
    ...
    try {
        const resp = await axios.post('api/get_aides_answers/', {
