<template >
    <ion-page>
        <ion-content :scroll-y="false">
            ...
            <filter-comp :is-open="fOpen" :typ-o="'aide'" :is-ent="true" :filt-obj="filtObj" @filt="e => (filtObj as any)[e.typ] = e.val"
...
const send_quest = async () => {
    ...
    if(filtObj.value.matiere == '' || filtObj.value.annee == '') {
    	return await show_alert('', 'Vous devez obligatoirement choisir une année d\'étude et une matiere pour votre question.')
    }
    
    const load = await showLoading('Envoi...')

    try {
        const resp = await axios.post('api/create_aides/' + (id_epr.value ? '?epr=' + id_epr.value  : ''), {
