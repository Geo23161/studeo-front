                    ...
                    <div v-for="ans in r_answers" :key="ans.id" class="quest_cont serif" :class="{
                        ...
                        <div style="margin-top: -1rem; min-width: 0; width: 100%;">
                            ...
                            <div class="quests">
                                ...
                                <div style="margin-top: .5rem; font-size: .9rem;" v-html="ans.text">
...
const my_work = computed(() => {
    return worko.value['aide' + id.value]
})
...
const get_aide = async () => {
    ...
    try {
        const resp = await axios.get('api/get_aide/' + id.value + '/', {
            headers: {
        ...
        if (resp.data['done']) {
            ...
            if (user.value.id == aide.value.user.id) interv.value = setInterval(() => {
                cpt.value++;
...
const set_working = async () => {
    ...
    try {
        ...
        load.dismiss()
        if (staff_w.value.working == user.value.id) router.push('/answer/' + id.value + '/?pre=' + (is_prem.value ? 'yes' : 'no'))
...
const get_answer = async () => {
    const resp = await axios.get('api/get_answer/' + id.value + '/?typ=aide', {
        headers: {
            Authorization: `Bearer ${await access_tok(router, undefined)}`
        }
    })
...
const set_satisfied = async () => {
    ...
    try {
        const resp = await axios.get('api/set_satisfied/' + aide.value?.id + '/?typ=aide', {
            headers: {
                Authorization: `Bearer ${await access_tok(router, load)}`
            }
        })
