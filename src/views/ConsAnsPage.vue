        ...
        <ion-content :scroll-y="false">
            <div class="all_cont serif">
                ...
                <div class="quests_cta">
                    <button @click="send_quest()" class="qcta">
            ...
            <filter-comp :is-open="fOpen" :typ-o="'conseil'" :is-ent="true" :filt-obj="filtObj"
                @filt="e => (filtObj as any)[e.typ] = e.val" @all_done="e => send_quest()"
                @close="fOpen = false"></filter-comp>
            <choose-comp :is-open="cOpen" @close="cOpen = false" />
            <recorder-comp :is-open="rOpen" @close="rOpen = false"
                @done="(e: any) => (audio = undefined, typ_file = 'aud', size = e.size, charg_file(undefined, e.blob), rOpen = false)" />
...
import { IonPage, IonContent, IonIcon, onIonViewDidEnter, onIonViewDidLeave } from "@ionic/vue"
...
const set_previews = (e: any) => {
    ...
    for (let i = 0; i < files.length; i++) {
        extract_frames(files[i], (b: Blob | null) => {
...
const send_quest = async () => {
    ...
    try {
        const resp = await axios.post('api/conseils_answers/', {
        ...
        if (resp.data['done']) {
            load.dismiss()
            router.replace('/quest/' + resp.data['result'] + '/')
        }
    } catch (e) {
        load.dismiss()
...
onIonViewDidEnter(() => {
    const pre = route.query['is_prem'] as string

    is_prem.value = pre == 'yes'
